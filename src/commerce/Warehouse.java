...
public class Warehouse {
    ...
    private void initWarehouse() { // 창고안에 카테고리안에 물건 쌓아놓는중
        ...
        clothingItem.addProduct(new Product("Hoodie", "오버핏 후드티", 59_000, "편안한 면 소재 후드", 50));
        clothingItem.addProduct(new Product("Jeans", "스트레이트 데님", 45_000, "사계절용 청바지", 30));
        categoryMap.put(2, clothingItem);

        // 3. 과자
        Category snacksItem = new Category("과자");
        snacksItem.addProduct(new Product("Potato Chips", "포테이토칩", 2_500, "바삭한 감자칩", 200));
        snacksItem.addProduct(new Product("Chocolate Bar", "초코바", 1_500, "에너지 충전용", 150));
        categoryMap.put(3, snacksItem);

        // 4. 생활용품
        Category dailyItem = new Category("생활용품");
        dailyItem.addProduct(new Product("Towel", "호텔 수건", 8_000, "도톰한 40수 면 수건", 60));
        dailyItem.addProduct(new Product("Tumbler", "텀블러", 25_000, "보온 성능 우수", 15));
        categoryMap.put(4, dailyItem);

        // 5. 가구
        Category furnitureItem = new Category("가구");
        furnitureItem.addProduct(new Product("Desk", "원목 책상", 150_000, "깔끔한 워크스테이션", 5));
        furnitureItem.addProduct(new Product("Office Chair", "사무용 의자", 120_000, "인체공학 의자", 8));
        categoryMap.put(5, furnitureItem);
    }
